...
import { Mail, Phone, MapPin, Send, Linkedin, Github, Copy, Check, Loader2 } from 'lucide-react';
...
const ContactSection = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    // apikey
    formData.append("access_key", "ca122fd4-cc58-47ee-84c1-287ca2e1cf51");
  ...
  const socials = [
    { Icon: Mail, href: "mailto:klaudia.mieczkowska01@gmail.com", label: "Gmail" },
    { Icon: Linkedin, href: "https://www.linkedin.com/in/klaudia-mieczkowska01/", label: "LinkedIn" },
    { Icon: Github, href: "https://github.com/kmieczkowska", label: "GitHub" }
  ];
      ...
      <div className="max-w-5xl mx-auto px-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-start">
                    ...
                    <div className="flex flex-col"> 
                      <div className="flex items-center gap-2">
                        ...
                        {item.label !== "Location" && (
                          ...
                          >
                            {copiedText === item.val ? <Check size={14} className="text-green-500" /> : <Copy size={14} className="opacity-0 group-hover:opacity-100 transition-opacity" />}
                          </button>
                        )}
                      </div>
                      {item.workMode && <p className="text-[11px] text-[#FFD700]/70 font-mono mt-0.5">{item.workMode}</p>}
                    </div>
                  </div>
                </motion.div>
              ))}
          ...
          >
            ...
            <form onSubmit={handleSubmit} className="relative bg-[#121212] border border-white/10 p-8 rounded-2xl shadow-2xl space-y-4">
              ...
              <motion.button 
                ...
                type="submit"
                className={`w-full font-bold py-4 rounded-lg flex items-center justify-center gap-2 transition-all group ${
                  status === "success" ? "bg-green-500 text-white" : "bg-[#FFD700] text-black hover:bg-[#ffdf33]"
                }`}
              >
                {status === "sending" ? (
                  <Loader2 className="animate-spin" size={18} />
                ) : status === "success" ? (
                  "Message sent to Klaudia!"
                ) : status === "error" ? (
                  "Error, try again"
                ) : (
                  <>
